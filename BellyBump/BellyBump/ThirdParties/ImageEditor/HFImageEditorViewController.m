...
@property(nonatomic, assign) CGFloat validRotationValue;
...
- (IBAction)doneAction:(id)sender
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            // Scale the image
            CGFloat scale = CGRectGetWidth(self.cropRect) * 2 / transform.size.width;
            transform = [BEBUtilities scaleImage:transform scaleFactor:scale];
            
            // Add caption for image
            UIImage *captionImage;
            if ([self respondsToSelector:@selector(addCaptionInImage:)]) {
                captionImage = [self addCaptionInImage:transform];
            }
            
            self.view.userInteractionEnabled = YES;
            
            if ([self.delegate respondsToSelector:@selector(imageEditor:finishWithImage:captionImage:cancel:)]){
                [self.delegate imageEditor:self finishWithImage:transform captionImage:captionImage cancel:NO];
            }
...
{
    if ([self.delegate respondsToSelector:@selector(imageEditor:finishWithImage:captionImage:cancel:)]){
        [self.delegate imageEditor:self finishWithImage:nil captionImage:nil cancel:YES];
    }
}
...
- (void)checkBoundsWithTransform:(CGAffineTransform)transform rotationValue:(CGFloat)value
{
    if(!self.checkBounds) {
        self.validTransform = transform;
        return;
...
- (void)handleRotationWithValue:(CGFloat)value;
{
    if (self.currentWidth == 0) {
        self.currentHeight = CGRectGetHeight([UIScreen mainScreen].bounds);
        self.currentWidth = CGRectGetWidth([UIScreen mainScreen].bounds);
    }

    CGAffineTransform transform =  CGAffineTransformTranslate(self.imageView.transform, 0, 0);
    
    transform = CGAffineTransformRotate(transform, (value - self.rotationValue) * M_PI/4);
    
    CGFloat ratio = self.rotationValue * 45 * M_PI / 180;
    if (ratio < 0) ratio *= -1;
    
    CGFloat newHeight = cos(ratio) * self.screenHeight +  sin(ratio) * self.screenWidth;
    CGFloat newWidth = cos(ratio) * self.screenWidth +  sin(ratio) * self.screenHeight;
    
    CGFloat scale = MAX(newHeight/self.currentHeight, newWidth/self.currentWidth);

    if (ratio > self.currentRatio) {
        if (newWidth > self.screenWidth * self.scale) {
    ...
    self.currentHeight = self.currentHeight * scale;
    self.currentWidth = self.currentWidth * scale;
    
    [self checkBoundsWithTransform:transform rotationValue:value];
...
- (void)rollBackCorrectTransform;
{
    ...
    [self resetSliderValueAtChildView: self.validRotationValue];
}
...
- (UIImage *)image:(UIImage *)image
       withCaption:(NSString *)text
          position:(CGPoint)position
              ...
              color:(UIColor *)color;
{
    return [BEBUtilities drawText:text
                          inImage:image
                          atPoint:position
                             font:font
                            color:color];
